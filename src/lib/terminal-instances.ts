...
export function getOrCreateTerminal(
  ...
  const { worktreeId, worktreePath, command = null, useWslOverride } = options
...
export async function attachToContainer(
  ...
  const { terminal, fitAddon, worktreePath, command, useWslOverride, initialized } = instance
    ...
    if (!initialized) {
      ...
      if (ptyExists) {
        ...
        await invoke('start_terminal', {
          ...
          rows,
          command,
          use_wsl_override: useWslOverride,
        }).catch(error => {
